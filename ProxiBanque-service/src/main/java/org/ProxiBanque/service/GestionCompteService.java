...
public class GestionCompteService {
	...
	public boolean virementCompteACompte(Courant comptedébiteur, Courant comptecréditeur, double somme) {
		...
		// On va chercher les possesseurs des comptes dans la base de donnée pour
		// pouvoir modifier par la suite leur solde total et leur situation financiere
		Client débiteur = new Client();
		débiteur.setIdClient(comptedébiteur.getIdClient());
		débiteur = clientDAO.getClient(débiteur);
		service.calculSommeTotal(débiteur);

		Client créditeur = new Client();
		créditeur.setIdClient(comptecréditeur.getIdClient());
		créditeur = clientDAO.getClient(créditeur);
		service.calculSommeTotal(créditeur);
